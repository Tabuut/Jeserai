...
class OpenAIGPT4Client:
    """
    OpenAI GPT-4o client for Iraqi dialect responses and image generation.
    Uses OpenRouter to access GPT-4o model with full image capabilities.
    """
    
    def __init__(self):
        """Initialize the OpenAI GPT-4o client."""
        self.api_key = os.environ.get("OPENAI_API_KEY")
        if not self.api_key:
            raise ValueError("OPENAI_API_KEY environment variable is not set")
        
        # Initialize OpenAI client with OpenRouter base URL for GPT-4o
        self.client = OpenAI(
...
"""
    ...
    async def generate_arabic_response(self, user_message: str) -> str:
        """
        Generate Arabic response using GPT-4o model.
        ...
        """
        try:
            logger.info(f"Sending request to GPT-4o for message: {user_message[:50]}...")
    ...
    async def analyze_image(self, image_data: bytes, user_message: str = "") -> str:
        ...
        """
        try:
            logger.info("Attempting image analysis with GPT-4o Vision...")
            ...
            if response.choices and response.choices[0].message.content:
                result = response.choices[0].message.content.strip()
                logger.info("Successfully analyzed image with GPT-4o Vision")
                return result
            else:
                logger.warning("Empty response from GPT-4o Vision")
    ...
    async def translate_to_arabic(self, english_text: str) -> str:
        """
        Translate English text to Iraqi Arabic using GPT-4o.
...
ابدأ الجواب بـ "ENGLISH_PROMPT:" ثم النص الإنجليزي، ثم سطر فارغ، ثم "ARABIC_DESCRIPTION:" ثم الوصف العراقي.
            ...
            if response.choices and response.choices[0].message.content:
                ...
                logger.info("Successfully generated creative description with GPT-4o")
                return arabic_description.strip(), english_prompt.strip()
            else:
                logger.warning("Empty response from GPT-4o for creative description")
